	...
	--*/
{
	...
	if (RegistryPath)
	{
		...
		if (ntStatus == STATUS_SUCCESS)
		{
			...
			BufDriverString = ExAllocatePoolWithTag(PagedPool, sizeof(KEY_VALUE_PARTIAL_INFORMATION) + 100, 0);
			BufDeviceString = ExAllocatePoolWithTag(PagedPool, sizeof(KEY_VALUE_PARTIAL_INFORMATION) + 100, 0);
			BufProcessEventString = ExAllocatePoolWithTag(PagedPool, sizeof(KEY_VALUE_PARTIAL_INFORMATION) + 100, 0);
			BufThreadEventString = ExAllocatePoolWithTag(PagedPool, sizeof(KEY_VALUE_PARTIAL_INFORMATION) + 100, 0);
